...
void solve_fields(grid *grid_all, double scale_x, double scale_y, double dt, int GRID_X, int GRID_Y)
{
	...
	for (i=move; i<move+GRID_X; i++)
	{
		for(j=0; j<GRID_Y; j++)
		{
			set=moving_frame_get(grid_all->grid_bx, i, j)-(moving_frame_get(grid_all->grid_ez, i, j)-moving_frame_get(grid_all->grid_ez, i, j-1))/dy*dt/2;
			moving_frame_set(grid_all->grid_bx, i, j, set);
			set=moving_frame_get(grid_all->grid_by, i, j)+(moving_frame_get(grid_all->grid_ez, i, j)-moving_frame_get(grid_all->grid_ez, i-1, j))/dx*dt/2;
			moving_frame_set(grid_all->grid_by, i, j, set);
			set=moving_frame_get(grid_all->grid_bz, i, j)-((moving_frame_get(grid_all->grid_ey, i, j)-moving_frame_get(grid_all->grid_ey, i-1, j))/dx-(moving_frame_get(grid_all->grid_ex, i, j)-moving_frame_get(grid_all->grid_ex, i, j-1))/dy)*dt/2;
...
void get_splits(split* split_data, int *splits, double x_start, double y_start, double x_end, double y_end, double scale_x, double scale_y)
{
	...
	if(*splits==0)
	{
		split_data->x0[0]=(x_start-(x_start_coordinate/scale_x))*scale_x;
		split_data->x1[0]=(x_end-(x_end_coordinate/scale_x))*scale_x;
		split_data->y0[0]=(y_start-(y_start_coordinate/scale_y))*scale_y;
		split_data->y1[0]=(y_end-(y_end_coordinate/scale_y))*scale_y;
	...
	if(*splits==1)
	{
		...
		if(x_start_coordinate-x_end_coordinate!=0)
		{
			...
			split_data->y1[0]=split_data->y0[0]+((delta_x*((y_end-y_start)*scale_y)/((x_end-x_start)*scale_x)));
			split_data->x0[1]=-split_data->x1[0];
			split_data->x1[1]=(x_end-(x_end_coordinate/scale_x))*scale_x;
			split_data->y0[1]=split_data->y1[0];
			split_data->y1[1]=(y_end-(y_end_coordinate/scale_y))*scale_y;
			if(x_end_coordinate>x_start_coordinate)
		...
		else
		{
			...
			split_data->x1[0]=split_data->x0[0]+(delta_y*((x_end-x_start)*scale_x)/((y_end-y_start)*scale_y));
			split_data->y0[1]=-split_data->y1[0];
			split_data->y1[1]=(y_end-(y_end_coordinate/scale_y))*scale_y;
			split_data->x0[1]=split_data->x1[0];
			split_data->x1[1]=(x_end-(x_end_coordinate/scale_x))*scale_x;
	...
	if(*splits==2)
	{
		...
		if(x_end-x_start<0)
			x_positive_delta=(x_start-x_end)*scale_x;
		else
			x_positive_delta=(x_end-x_start)*scale_x;
		if(y_end-y_start<0)
			y_positive_delta=(y_start-y_end)*scale_y;
		else
			y_positive_delta=(y_end-y_start)*scale_y;
		...
		if(((0.5-y_start_help)/(0.5-x_start_help))>((y_positive_delta)/(x_positive_delta)))
		{
			...
			split_data->x1[1]=split_data->x0[1]+(delta_y*((x_end-x_start)*scale_x)/((y_end-y_start)*scale_y));
			if(x_end_coordinate>x_start_coordinate)
				split_data->i_cell[1]=split_data->i_cell[0]+1;
			else
				split_data->i_cell[1]=split_data->i_cell[0]-1;
			...
			split_data->x1[2]=(x_end-(x_end_coordinate/scale_x))*scale_x;
			split_data->y1[2]=(y_end-(y_end_coordinate/scale_y))*scale_y;
		...
		else
		{
			...
			split_data->y1[1]=split_data->y0[1]+(delta_x*((y_end-y_start)*scale_y)/((x_end-x_start)*scale_x));
			split_data->i_cell[1]=split_data->i_cell[0];
			if(y_end_coordinate>y_start_coordinate)
				split_data->j_cell[1]=split_data->j_cell[0]+1;
			else
...
void current_deposition (grid *grid_all, double x_start, double y_start, double x_end, double y_end, double q, double dt, double scale_x, double scale_y, int GRID_X, int GRID_Y, double pz, double q_weight)
{
	...
	double jnorm_x=(1/scale_x)/(2*dt);
	double jnorm_y=(1/scale_y)/(2*dt);
